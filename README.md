...
| `COLLUSIVE_CLEANUP_ENABLED` | `1` | 启用启动/请求前缓存保留期清理 |
| `COLLUSIVE_CLEANUP_INTERVAL_SECONDS` | `3600` | 请求前清理扫描最小间隔，单位秒 |
| `COLLUSIVE_DOWNLOAD_RETENTION_DAYS` | `30` | `batch_downloads` 批次目录保留天数，`<=0` 表示不清理 |
| `COLLUSIVE_WORKDIR_RETENTION_DAYS` | `7` | `collusive_workdir/batch_*` 工作目录保留天数，`<=0` 表示不清理 |
...
| `BID_TEMPLATE_BATCH_DOC_RATIO` | `0.30` | 批次高频模板段落的文档占比阈值 |
| `BID_TEMPLATE_BATCH_MIN_DOCS` | `3` | 批次高频模板段落的最小出现文档数 |
...
API 启动时会扫描一次缓存目录；请求进入时按 `COLLUSIVE_CLEANUP_INTERVAL_SECONDS` 控制扫描频率。正在执行的 `batchId` 和当前请求的 `batchId` 会跳过清理。

| 目录 | 默认保留期 | 环境变量 | 说明 |
| :--- | :--- | :--- | :--- |
| `batch_downloads/{batchId}` | 30 天 | `COLLUSIVE_DOWNLOAD_RETENTION_DAYS` | PDF 下载缓存，同一 batchId 重复检查时复用 |
| `collusive_workdir/batch_*` | 7 天 | `COLLUSIVE_WORKDIR_RETENTION_DAYS` | 管线工作目录、SQLite 特征库、输出 JSON、异常报告 |

`collusive_workdir/shared_embedding_cache` 不属于 `batch_*` 工作目录，不会被该策略删除。保留天数设置为 `0` 或负数时，对应目录不执行过期清理。
...
COLLUSIVE_DOWNLOAD_RETENTION_DAYS=30
COLLUSIVE_WORKDIR_RETENTION_DAYS=7
COLLUSIVE_CLEANUP_INTERVAL_SECONDS=3600
BID_TEMPLATE_HARD_FILTER=1
BID_TEMPLATE_RATIO_THRESHOLD=0.55
BID_TEMPLATE_MIN_INFO_SCORE=0.28
```

### 模板语言过滤

技术标/商务标文本相似检测会对段落计算三类证据价值：

| 指标 | 说明 |
| :--- | :--- |
| `boilerplate_ratio` | 模板语、招标文件原文、格式字段在段落中的覆盖比例 |
| `informativeness_score` | 数字、型号、参数、金额、专有词等项目特异信息密度 |
| `batch_common_template` | 当前批次同维度下多家公司重复出现的高频段落 |

满足“高模板比例 + 低信息量”的段落不进入 `similarParagraphs`，也不贡献文本雷同状态。当前批次高频模板段落按技术标/商务标分别统计，不会跨维度复用。
